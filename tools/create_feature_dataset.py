...
def create_structure_dataset(src_root_dir:str, dst_root_dir:str, filename_history:str, mode:Literal["train", "test"]="train"):
  ...
  progress_bar = tqdm.tqdm(total=tqdm_len, desc="copy renamed images") 
  ...
  progress_bar.close()
  progress_bar = tqdm.tqdm(total=len(listdir(f"{src_root_dir}/labels")), desc="copy renamed origin annotations") 
  ...
  # 주의!!!!
  # 이거 개발할 땐 data/images/anger... data/labels/anger... 이런식의 구조에서 짬
  progress_bar = tqdm.tqdm(total=len(listdir(f"{src_root_dir}/labels")), desc="create coco annotations") 
